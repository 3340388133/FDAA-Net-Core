...
"""
跨数据集泛化实验 v3
策略改进: 将NTIRE2026纳入训练集，替换CIFAKE
  - BigGAN (GAN, ImageNet-scale)
  - ADM (Diffusion, ImageNet-scale)
  - NTIRE2026 (多样化、in-the-wild、多种生成器)
测试: glide, VQDM, CIFAKE, DiffusionForensics (4个完全未见数据集)
改进: 训练增加JPEG增强和高斯模糊，提升鲁棒泛化
"""
...
def main():
    ...
    print("测试: glide, VQDM, CIFAKE, DiffForensics (4个完全未见数据集)")
    print("改进: 训练增加JPEG增强+高斯模糊; NTIRE2026平衡采样")
    print("="*70)
    ...
    # ===== 测试 =====
    print(f"\n{'='*70}")
    print(f"跨数据集泛化测试结果 (v3)")
    print(f"训练: BigGAN + ADM + NTIRE2026(shard_4,5)")
    print(f"测试: glide, VQDM, CIFAKE, DiffForensics + JPEG75")
    print(f"{'='*70}")
    ...
    output = {
        'experiment': 'cross_dataset_v3',
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'config': {
            'train_sources': ['BigGAN', 'ADM', 'NTIRE2026(shard_4,5)'],
    ...
    with open(out_dir / 'cross_dataset_v3_results.json', 'w', encoding='utf-8') as f:
        json.dump(output, f, indent=2, ensure_ascii=False)

    with open(out_dir / 'cross_dataset_v3_results.csv', 'w', newline='') as f:
        w = csv.writer(f)
    ...
    print(f"\n结果已保存: outputs/cross_dataset_v3_results.json / .csv")
