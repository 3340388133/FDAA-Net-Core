...
策略: 4源训练，3个未见数据集测试
训练:
  - BigGAN (GAN, ImageNet-scale)
  - ADM (Diffusion, ImageNet-scale)
  - CIFAKE (CIFAR-10 scale, SD生成)
  - NTIRE2026 (多样化、in-the-wild、多种生成器)
测试: glide, VQDM, DiffusionForensics (3个完全未见数据集)
改进: 训练增加JPEG增强和高斯模糊，提升鲁棒泛化; 4源多样化训练提升泛化
"""
...
def main():
    ...
    print("测试: glide, VQDM, DiffForensics (3个完全未见数据集)")
    print("改进: 4源多样化训练; JPEG增强+高斯模糊; NTIRE2026平衡采样")
    print("="*70)
    ...
    # BigGAN
    print("  加载 BigGAN...")
    ds_bg = GenImageDataset(f"{BASE}/GenImage/BigGAN/imagenet_ai_0419_biggan",
                            'train', train_tf, TRAIN_SAMPLES)
    try:
        vds_bg = GenImageDataset(f"{BASE}/GenImage/BigGAN/imagenet_ai_0419_biggan",
                                  'val', test_tf, 2000)
    except:
        vds_bg = GenImageDataset(f"{BASE}/GenImage/BigGAN/imagenet_ai_0419_biggan",
                                  'train', test_tf, 2000)
    ...
    # ADM
    print("  加载 ADM...")
    ds_adm = GenImageDataset(f"{BASE}/GenImage/ADM/imagenet_ai_0508_adm",
                              'train', train_tf, TRAIN_SAMPLES)
    try:
        vds_adm = GenImageDataset(f"{BASE}/GenImage/ADM/imagenet_ai_0508_adm",
                                    'val', test_tf, 2000)
    except:
        vds_adm = GenImageDataset(f"{BASE}/GenImage/ADM/imagenet_ai_0508_adm",
                                    'train', test_tf, 2000)
    ...
    # CIFAKE
    print("  加载 CIFAKE...")
    ds_cifake = CIFAKEDataset(f"{BASE}/CIFAKE", split='train', transform=train_tf,
                               max_samples=TRAIN_SAMPLES)
    vds_cifake = CIFAKEDataset(f"{BASE}/CIFAKE", split='test', transform=test_tf,
                                max_samples=2000)
    ...
    # 域内基准
    for name, desc in [('BigGAN', 'genimage'), ('ADM', 'genimage')]:
        path = f"{BASE}/GenImage/{'BigGAN/imagenet_ai_0419_biggan' if name=='BigGAN' else 'ADM/imagenet_ai_0508_adm'}"
        try:
            ds = load_test_dataset(name, path, desc, test_tf, TEST_SAMPLES)
    ...
    # CIFAKE域内测试
    try:
        ds_cifake_test = CIFAKEDataset(f"{BASE}/CIFAKE", split='test', transform=test_tf,
    ...
    # ===== 测试 =====
    print(f"\n{'='*70}")
    print(f"跨数据集泛化测试结果 (v5)")
    print(f"训练: BigGAN + ADM + CIFAKE + NTIRE2026(shard_4,5)")
    print(f"测试: glide, VQDM, DiffForensics + JPEG75")
    print(f"{'='*70}")
    ...
    with open(out_dir / 'cross_dataset_v5_results.json', 'w', encoding='utf-8') as f:
        json.dump(output, f, indent=2, ensure_ascii=False)

    with open(out_dir / 'cross_dataset_v5_results.csv', 'w', newline='') as f:
        w = csv.writer(f)
    ...
    print(f"\n结果已保存: outputs/cross_dataset_v5_results.json / .csv")
