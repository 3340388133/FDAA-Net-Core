#!/usr/bin/env python3
"""
消融实验：验证FDAA和MGFP模块的独立贡献
4组实验：
  1. Baseline: 预训练ViT + 分类头 (无FDAA, 无MGFP)
  2. +FDAA: 预训练ViT + FDAA + 分类头
  3. +MGFP: 预训练ViT + MGFP + 分类头
  4. Full: 预训练ViT + FDAA + MGFP + 分类头 (完整模型)
"""
...
class AblationModel(nn.Module):
    """消融实验用模型，可独立控制FDAA和MGFP"""
    def __init__(self, num_classes=2, embed_dim=768, num_prototypes=4,
                 dropout=0.1, use_fdaa=True, use_mgfp=True):
        super().__init__()
        ...
        if use_fdaa:
            self.fdaa = FDAA(
                dim=embed_dim, img_size=self.img_size,
    ...
    def forward(self, image, return_attention=False):
        ...
        # MGFP
        if self.use_mgfp:
            output, aux = self.mgfp(cls_token, patch_tokens, return_attention=True)
        else:
            output = cls_token
...
def main():
    ...
    for variant_name, use_fdaa, use_mgfp in ablation_configs:
        ...
        print(f"  FDAA: {'✓' if use_fdaa else '✗'}  MGFP: {'✓' if use_mgfp else '✗'}")
        print(f"{'='*60}")

        model = AblationModel(
            num_classes=2, embed_dim=768, num_prototypes=4,
            dropout=0.1, use_fdaa=use_fdaa, use_mgfp=use_mgfp
        ).to(device)
        ...
        print(f"  参数: {total_p:,} (可训练: {trainable_p:,})")
    ...
    header = f"{'Variant':25s} {'FDAA':>5s} {'MGFP':>5s}"
    ...
    for variant_name, use_fdaa, use_mgfp in ablation_configs:
        ...
        row = f"{variant_name:25s} {'✓':>5s if use_fdaa else '✗':>5s} {'✓':>5s if use_mgfp else '✗':>5s}"
        for test_name in domain_names:
            row += f" {r[test_name]['auc']:15.4f}"
    ...
    for variant_name, use_fdaa, use_mgfp in ablation_configs:
        r = all_results[variant_name]
        row = {'variant': variant_name, 'use_fdaa': use_fdaa, 'use_mgfp': use_mgfp,
               'params': r['params']}
        for test_name in domain_names:
        ...
        rows.append(row)
    pd.DataFrame(rows).to_csv('outputs/ablation_results.csv', index=False)

    print(f"\n结果已保存: outputs/ablation_results.json / .csv")
