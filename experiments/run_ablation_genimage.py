...
使用本地 GenImage 数据集运行消融实验

Usage:
    LD_LIBRARY_PATH="" python experiments/run_ablation_genimage.py
    LD_LIBRARY_PATH="" python experiments/run_ablation_genimage.py --epochs 5 --configs baseline,full
"""
...
from data.genimage_dataset import create_genimage_dataloader
...
def main(args):
    print("="*60)
    print("AIGC Detection - GenImage 消融实验")
    print("="*60)
    ...
    result_file = os.path.join(args.output_dir, f"ablation_genimage_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run ablation experiments on GenImage')

    # 数据集
    parser.add_argument('--data_root', type=str,
                        default='datasets/genimage_partial/imagenet_ai_0419_biggan',
                        help='GenImage dataset root directory')
