"""
AIGC Image Detection Model
整合FDAA和MGFP的完整检测器

基于CLIP ViT backbone，融合两个创新点:
1. FDAA: 频率感知双域自适应Adapter
2. MGFP: 多粒度伪造感知模块

版本:
- AIGCDetector (V1): 原始架构，FDAA注入ViT token
- AIGCDetectorV2: Late Fusion架构，独立频率分支（主模型）
- AIGCDetectorLite: 轻量版（支持V1/V2切换）

重要: CLIP ViT-L/14 内部维度为 1024，embed_dim 应设为 1024。
"""
