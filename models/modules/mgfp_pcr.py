"""
Multi-Granularity Forgery Perception with Prototype Contrastive Regularization (MGFP-PCR)
多粒度伪造感知模块 + 原型对比正则化

核心改进：
1. 原型对比正则化：鼓励原型捕获跨域通用的伪造模式
2. 域不变原型学习：通过对比学习使原型与具体生成器无关
3. 动态原型更新：使用动量更新策略保持原型稳定性
4. 层级原型：不同粒度的原型捕获不同尺度的伪造特征
"""
